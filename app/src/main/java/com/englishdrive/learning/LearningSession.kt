...
class LearningSession(private val context: Context) {
    ...
    fun levelUp() {
        if (levelManager.levelUp()) {
            ...
            if (isSessionActive) {
                sendToGemini(
                    "The student moved up to level ${levelManager.currentLevelNumber}/100. " +
                    "Congratulate them and start teaching at this new level."
                )
            }
        } else {
            if (isSessionActive) {
                emmaSpeak("You're already at level 100 — native fluency! Brilliant.") {
                    startListening()
                }
            }
        }
        ...
        if (levelManager.levelDown()) {
            ...
            if (isSessionActive) {
                sendToGemini(
                    ...
                    "Be kind and restart at this level."
                )
            }
        } else {
            if (isSessionActive) {
                emmaSpeak("You're already at level 1. Let's keep going from here!") {
                    startListening()
                }
            }
        }
    ...
    fun changeLevel(n: Int) {
        ...
        if (isSessionActive) {
            sendToGemini(
                "The student changed to level ${levelManager.currentLevelNumber}/100. " +
    ...
    fun handleExternalCommand(cmd: String) {
        ...
        when (cmd) {
            ...
            "stats" -> {
                ...
                emmaSpeak("You're at level ${level.number} out of 100. ${level.shortName}.") {
                    startListening()
                }
            }
            "levelup"   -> levelUp()
    ...
    private fun handleStudentInput(text: String) {
        ...
        when (val cmd = VoiceCommandParser.parse(text)) {
            is VoiceCommandParser.Command.SetLevel -> changeLevel(cmd.levelNumber)
            ...
            VoiceCommandParser.Command.StopSession -> {
                emmaSpeak("Great work today. See you next time!") { stopSession() }
            }
            VoiceCommandParser.Command.PauseSession -> {
                emmaSpeak("Pausing now. Say resume when ready.") { pauseSession() }
            }
            ...
            VoiceCommandParser.Command.RepeatLast -> {
                if (lastEmmaMessage.isNotBlank()) {
                    ...
                    emmaSpeak("I haven't said anything yet!") { startListening() }
                }
            }
            ...
            VoiceCommandParser.Command.SpeakSlower -> {
                ...
                emmaSpeak("Of course, I'll speak more slowly.") { startListening() }
            }
    ...
    private fun sendToGemini(raw: String) {
        ...
        val isStart = raw == "__START__"
        val msg = if (isStart) "Hello" else raw
        scope.launch {
